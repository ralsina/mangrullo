...
docker run --rm -v "$PWD":/app --user="$UID" mangrullo-builder /bin/sh -c "cd /app && rm -rf lib shard.lock && shards build --without-development --static"
...
docker run --rm -v "$PWD":/app --platform linux/arm64 --user="$UID" mangrullo-builder /bin/sh -c "cd /app && rm -rf lib shard.lock && shards build --without-development --static"
