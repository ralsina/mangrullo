module Mangrullo
  ...
  class UpdateJobQueue
    ...
    # Get all jobs
    def get_all_jobs : Array(UpdateJob)
      @mutex.synchronize do
        @jobs.values.to_a
      end
    end
    ...
    private def process_next_job
      job = @mutex.synchronize do
        ...
        next_job = @jobs.values.find { |job| job.status == JobStatus::Pending }
...
struct UUID
  def self.random : String
    ...
    # Format as UUID v4
    String.build do |io|
      bytes[0, 4].each { |b| io.printf "%02x", b }
      io << '-'
      bytes[4, 2].each { |b| io.printf "%02x", b }
      io << '-'
      io.printf "%02x%02x", bytes[6] & 0x0f | 0x40, bytes[7]
      io << '-'
      io.printf "%02x%02x", bytes[8] & 0x3f | 0x80, bytes[9]
      io << '-'
      bytes[10, 6].each { |b| io.printf "%02x", b }
    end
  end
end